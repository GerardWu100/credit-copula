...
rating_data = pd.DataFrame(
    {"Rating": ratings, "Probability (%)": probabilities * 100, "Value ($)": values}
)
...
losses_copula = np.column_stack(
    (initial_bond_value - bond1_values_copula, initial_bond_value - bond2_values_copula)
)
...
spearman_mvn = stats.spearmanr(
    losses_mvn[diagnostic_idx, 0], losses_mvn[diagnostic_idx, 1]
).correlation
spearman_copula = stats.spearmanr(
    losses_copula[diagnostic_idx, 0], losses_copula[diagnostic_idx, 1]
).correlation
...
ax1 = axes[0, 0]
bins = np.linspace(
    min(credit_losses_mvn.min(), credit_losses_copula.min()),
    max(credit_losses_mvn.max(), credit_losses_copula.max()),
    150,
)
ax1.hist(
    credit_losses_mvn,
...
ax1.axvline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"A: Mean=${expected_loss_mvn:.2f}",
)
ax1.axvline(
    expected_loss_copula,
    color="red",
    linestyle="--",
    linewidth=2,
    label=f"B: Mean=${expected_loss_copula:.2f}",
)
...
width = 0.35
ax2.bar(
    x_pos - width / 2, var_mvn, width, label="Method A", color="steelblue", alpha=0.8
)
ax2.bar(
    x_pos + width / 2, var_copula, width, label="Method B", color="coral", alpha=0.8
)
...
ax3.axvline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"A: 99% VaR=${var_99_mvn:.2f}",
)
ax3.axvline(
    var_99_copula,
    color="red",
    linestyle="--",
    linewidth=2,
    label=f"B: 99% VaR=${var_99_copula:.2f}",
)
...
plt.tight_layout()
plt.savefig(
    output_dir / "001_credit_risk_comparison_overview.png", dpi=300, bbox_inches="tight"
)
...
ax.set_title(
    "Portfolio Loss Distribution: MVN vs Gaussian Copula",
    fontsize=14,
    fontweight="bold",
    pad=15,
)
...
ax.set_title(
    "Value-at-Risk Comparison Across Confidence Levels",
    fontsize=14,
    fontweight="bold",
    pad=15,
)
...
ax.set_ylabel("Probability Density", fontsize=13, fontweight="bold")
ax.set_title(
    "Tail Risk Distribution (>90th Percentile)", fontsize=14, fontweight="bold", pad=15
)
ax.legend(fontsize=11, loc="upper right", framealpha=0.95)
ax.grid(True, alpha=0.3, linestyle="--")
ax.tick_params(labelsize=11)
...
for n_bonds in bond_counts:
    ...
    print(
        f"n={n_bonds:3d} bonds | Copula 99% VaR: ${var_copula:10.2f} | MVN 99% VaR: ${var_mvn:10.2f}"
    )
...
scaling_results = pd.DataFrame(
    {
        ...
        "Copula VaR per Bond ($)": [
            v / n for v, n in zip(var_99_copula_by_size, bond_counts)
        ],
        "MVN VaR per Bond ($)": [
            v / n for v, n in zip(var_99_mvn_by_size, bond_counts)
        ],
    }
)
...
ax1.annotate(
    f"10 bonds: ${var_99_copula_by_size[bond_counts.index(10)]:.0f}",
...
ax2.set_title(
    "Diversification Effect: VaR per Bond", fontsize=14, fontweight="bold", pad=15
)
